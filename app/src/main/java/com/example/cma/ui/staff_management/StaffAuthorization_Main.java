...
public class StaffAuthorization_Main extends AppCompatActivity {
    public List<StaffAuthorization> dangAns = new ArrayList<StaffAuthorization>();
    private StaffAuthorizationAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.staff_authorization__main);
        initView();
        initDangan();
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Intent intent = new Intent(StaffAuthorization_Main.this, StaffAuthorization_See.class);
                StaffAuthorization d = (StaffAuthorization) listView.getItemAtPosition(i);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(StaffAuthorization_Main.this, StaffAuthorization_Add.class);
                startActivity(intent);
            }
        });
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    // String url;
                    // if(preId!=-1)
                    //   url="http://119.23.38.100:8080/cma/StaffQualification"+
                    Log.d("点击获取的", "here is json11");
                    OkHttpClient client = new OkHttpClient();
                    Request request = new Request.Builder()
                            // 指定访问的服务器地址，后续在这里修改
                            .url("http://119.23.38.100:8080/cma/StaffAuthorization/getAll")
...
//        ReceiveData receiveData=gson.fromJson(jsonData,new TypeToken<ReceiveData>(){}.getType());
        ...
        List<StaffAuthorization> temp = new ArrayList<StaffAuthorization>();
        temp = gson.fromJson(string, new TypeToken<List<StaffAuthorization>>() {
        }.getType());
        if (preId != -1) {
            dangAns.clear();
            for (StaffAuthorization t : temp) {
                if (t.getId() == preId)
                    dangAns.add(t);
            }
        } else
            dangAns = gson.fromJson(string, new TypeToken<List<StaffAuthorization>>() {
            }.getType());


        //dangAns=receiveData.getData();
        // dangAns = gson.fromJson(jsonData, new TypeToken<List<StaffAuthorization>>(){}.getType());
    ...
    private void showResponse() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                // 在这里进行UI操作，将结果显示到界面上
                adapter = new StaffAuthorizationAdapter(StaffAuthorization_Main.this, R.layout.staff_authorization_listview, dangAns);
                listView.setAdapter(adapter);
            }
        });
    }
